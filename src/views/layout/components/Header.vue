<template>
  <div class="layout-header__com">
    <el-header>
      ...
      <div class="operation">
        <span class="item">
          <router-link to="/add" tag="span"> 加好友 <i class="el-icon-plus"></i> </router-link>
        </span>
        <span class="item">
          <router-link to="/mzone" tag="span"> 发动态 <i class="el-icon-plus"></i> </router-link>
        </span>
      </div>
      <div class="user-info">
        <el-dropdown class="droplist">
          <span class="el-dropdown-link">
            <el-badge :is-dot="validateUnReadCount > 0" class="badge-item">
              <el-avatar size="large" class="avatar" :src="IMG_URL + userInfo.photo" @error="() => true">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item class="user-menu-item">
              <router-link to="/setting" class="link"> 个人设置 </router-link>
            </el-dropdown-item>
            <el-dropdown-item class="user-menu-item">
              <el-badge :value="validateUnReadCount" :hidden="validateUnReadCount === 0">
                <router-link to="/system" class="link"> 系统消息 </router-link>
              </el-badge>
            </el-dropdown-item>
            <el-dropdown-item class="user-menu-item">
              <router-link to="/setting" class="link"> 主题设置 </router-link>
            </el-dropdown-item>
            <el-dropdown-item class="user-menu-item">
              <router-link to="/setting" class="link"> 反馈 </router-link>
            </el-dropdown-item>
            <el-dropdown-item class="user-menu-item">
              <a class="link" @click="logout"> 退出 </a>
            </el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
    ...
    <transition name="fade">
      <vue-draggable-resizable v-if="isToCoArtBoard" drag-cancel=".drawingarea">
    ...
    <transition name="fade">
      <vue-draggable-resizable v-if="isVideoing || isAudioing" :x="0" :y="500" drag-cancel=".drawingarea">
        <div class="co-art-board box-shadow1">
          <co-video :currentconversation="currentConversation" :state="webRTCState" :web-rtc-type="coVideoWebRtcType" />
        </div>
      </vue-draggable-resizable>
    </transition>
  </div>
...
export default {
  ...
  created() {
    this.$eventBus.$on('hangup', () => {
      this.webRTCState = 'apply' //白板结束后设置为初始化状态
    })
  }
}
</script>
